  ...
  <div class="col-md-6">
    ...
    {!! Form::open(['route'=>'pembayaran.store', 'role'=>'form', 'class'=>'form-horizontal', 'id'=>'form-create-menu']) !!}
            <div class="form-group">
                ...
                <div class="col-sm-10">
                <input id="input_name" type="name" name="name" value="{{ $customer[0]->name }}" style="width:50%" class="form-control">
                </div>
            </div>
            <div class="form-group">
                <label class="control-label col-sm-2" for="hp">No. HP </label>
                <div class="col-sm-10">
                <input id="input_hp" type="hp" name="hp" value="{{ $customer[0]->phone }}" style="width:50%" class="form-control">
                </div>
            </div>
            <div class="form-group">
                <label class="control-label col-sm-2" for="email">Email </label>
                <div class="col-sm-10">
                <input id="input_email" type="email" name="email" value="{{ $customer_email->email }}" style="width:50%" class="form-control">
                </div>
            </div>
            <br>
    ...
    <div class="table-responsive">
      <table class="table">
        ...
        <tbody>
          ...
          <script>$(document).ready(function(c) {
           ...
           <tr>
             ...
             <td><p>IDR <span class="sum_price_class">{{ \DB::table('products')->where('id',$product['product_id'])->value('amount') }}</span></p></td>
             <td>
             ...
             <td>
              <input id="price{{ $product['product_id'] }}" name="price_per_item[]" type="hidden" value="{{ \DB::table('products')->where('id',$product['product_id'])->value('amount') }}">
               <p id="sum_price{{ $product['product_id'] }}">IDR <span class="sum_price_class">{{ \DB::table('products')->where('id',$product['product_id'])->value('amount')*$product['quantity'] }}</span></p></td>
               <?php
                $sum_all += \DB::table('products')->where('id',$product['product_id'])->value('amount')*$product['quantity'];
               ?>
           </tr>
          @endforeach
        </tbody>
        <tfoot>
          <tr>
            <td>Postal Fee</td>
            <td>IDR <span id="postal_fee"></span></td>
            <td>Amount</td>
            <td>IDR <span id="sum_all">{{ $sum_all }}</span></td>
          </tr>
        </tfoot>
      </table>
    </div>
    ...
    // Fungsi dimulai...
      ...
      $('.copy').hide();
        ...
        $('#jne').on('click',function(){
          $('#postal_fee').text('9.000,00');
        })
        $('#jnt').on('click',function(){
          $('#postal_fee').text('10.000,00');
        })
        $('#gojek').on('click',function(){
          $('#postal_fee').text('15.000,00');
        })
